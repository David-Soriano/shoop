...
class Pedido
{
    ...
    {
        $res = NULL;
        $sql = "SELECT p.idped, p.idusu, p.total, p.fecha, p.estped, dp.iddet, dp.idped, dp.idpro, dp.cantidad, (SELECT SUM(dp2.cantidad) 
     FROM detalle_pedido dp2 WHERE dp2.idped = p.idped) AS total_productos, dp.precio AS precio_pedido, dp.mpago, dp.npago, cp.fechareg, pr.idpro, pr.nompro, pr.precio AS precio_producto, pr.pordescu, pr.idval, v.nomval, i.imgpro, i.nomimg, (pr.precio * (pr.pordescu / 100)) AS valor_descuento, (pr.precio - (pr.precio * (pr.pordescu / 100))) AS precio_final, pr.valorunitario, pv.idprov, pv.nomprov, pv.desprv FROM pedido p INNER JOIN detalle_pedido dp ON p.idped = dp.idped LEFT JOIN compra AS cp ON p.idped = cp.idped INNER JOIN producto pr ON pr.idpro = dp.idpro INNER JOIN valor v ON v.idval = pr.idval INNER JOIN prodxprov AS pxp ON pxp.idpro = pr.idpro INNER JOIN proveedor AS pv ON pxp.idprov = pv.idprov LEFT JOIN ( SELECT idpro, imgpro, nomimg FROM imagen WHERE (idpro, ordimg) IN ( SELECT idpro, MIN(ordimg) FROM imagen GROUP BY idpro ) ) i ON pr.idpro = i.idpro WHERE p.idped = :idped;";
